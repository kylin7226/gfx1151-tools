...
| 子项目 | 说明 |
|--------|------|
| [vllm/](vllm/) | vLLM 文本大模型推理（Qwen 3.6-27B AWQ-INT4）+ ASR + TTS |
...
| vllm-tts | 8003 | Qwen3-TTS-1.7B | 文字转语音（OpenAI 兼容 `/v1/audio/speech`） |
...
> 详细的容器运行命令（包括独立 `podman run`、docker compose、GHCR 预构建镜像拉取）见 [vllm/docs/RUN.md](vllm/docs/RUN.md)。
...
| 推理引擎 | vLLM 0.20.1 / vLLM-Omni 0.20.0rc1 | 源码 + 20 个补丁（详见 [vllm/docs/PATCHES.md](vllm/docs/PATCHES.md)） |
...
│   ├── glados.py          ← CLI 客户端
│   ├── docs/              ← vLLM 文档（LLM/ASR/使用指南/容器运行速查）
...
| **本项目（gfx1151-tools）** | Ubuntu 生产部署 | **v0.20.1 + 20 个补丁** | 多服务编排（LLM+ASR+Omni+TTS）、TheRock ROCm SDK |
...
**与本项目关系**：本项目的 20 个补丁中有 12 个来自 kyuz0 的 toolboxes 项目，而 kyuz0 的工作又基于 ROCm/vllm 分支的功能。ROCm/vllm 是底层内核的源头，但本项目的补丁更聚焦于让生产环境（Docker + 多服务）稳定运行，而非提供完整的内核实现。
...
|------|-----------------|--------|
| OS | Fedora 43 + TheRock tarball | Ubuntu 24.04 + TheRock tarball |
| 容器 | Toolbx（交互式桌面环境） | Podman（headless 服务） |
| 量化 | BF16 + AWQ（bitsandbytes） | AWQ-INT4（compressed-tensors） |
| 网络 | RDMA/RoCE 多节点集群 | 单节点 |
| 服务 | 单 vLLM 实例 | LLM + ASR + Omni + TTS 四服务 |
| 补丁数量 | 10+（含 AITER 头文件替换） | 20（含 profile 缓存、API 修复、CMake 修复） |
...
| 服务编排 | 单 vLLM 服务 | LLM + ASR + Omni + TTS 四服务 |
| OS | Ubuntu 24.04 | Ubuntu 24.04 |
| ROCm SDK | pip 7.13 nightly | TheRock 7.13 nightly tarball |
| 文档 | 英文 | 中文 |
...
- **多服务共存**：LLM（8000）+ ASR（8001）+ Omni（8002）+ TTS（8003）四服务独立运行，各自优化
- **TheRock ROCm SDK**：采用 TheRock tarball 替代 pip 安装包，更轻量、更可复现
...
| 文档 | 内容 |
|------|------|
| [vllm/docs/RUN.md](vllm/docs/RUN.md) | 容器运行与模型启动速查（docker compose / podman run / GHCR 镜像） |
| [vllm/docs/GUIDE.md](vllm/docs/GUIDE.md) | 从零开始的全流程使用指南 |
| [vllm/docs/LLM.md](vllm/docs/LLM.md) | Qwen3.6-27B 文本大模型详细部署指南 |
| [vllm/docs/ASR.md](vllm/docs/ASR.md) | Qwen3-ASR 语音识别详细部署指南 |
| [vllm/docs/PATCHES.md](vllm/docs/PATCHES.md) | 20 个补丁逐条分析与可移除性评估 |
| [vllm/README.md](vllm/README.md) | vLLM 子项目概览 |
| [vllm-omni/README.md](vllm-omni/README.md) | vLLM-Omni 子项目概览 |
